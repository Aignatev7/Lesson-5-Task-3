...
public:
	...
	virtual void print_myself() {
		std::cout << get_figure_name() << ": \n" << (true ? "Правильная" : "Неправильная") << std::endl;
...
public:
	...
	void print_myself() override {
		...
		std::cout << "Углы: " << "А=" << get_corner_A() << " B=" << get_corner_B() << " C=" << get_corner_C() << "\n" << std::endl;
	}
};
...
public:
	...
	bool check() override {
		if ((side_a == side_b == side_c) && (corner_A == corner_B == corner_C == 90)) {
			return true;
		}
	}
};
...
public:
	...
	void print_myself() override {
		std::cout << get_figure_name() << ": \n" << (number_of_sides ? "Правильная" : "Неправильная") << std::endl;
		std::cout << "Количество сторон: " << get_number_of_sides() << std::endl;
		std::cout << "Стороны: " << "a=" << get_side_a() << " b=" << get_side_b()
